...
internal enum class NativeDistributionType(val suffix: String?, val containsPlatformLibs: Boolean, val canGeneratePlatformLibs: Boolean) {
    LIGHT(null, false, true),
    PREBUILT("prebuilt", true, true),

    // Distribution types for 1.3:
    LIGHT_1_3("restricted", false, false),
    PREBUILT_1_3(null, true, false)
}

internal class NativeDistributionTypeProvider(private val project: Project) {
    private val propertiesProvider = PropertiesProvider(project)

    private fun warning(message: String) = SingleWarningPerBuild.show(project, message)
